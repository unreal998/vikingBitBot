//TEST
const SERVER_URL = 'https://localhost:3003'

const EMOJI_NAMES = {
    MY_LINKS: 'myLinks',
    REMOVE_LINK: 'delete',
    SETTINGS: 'settings',
    CREATE_LINK: 'createLink',
    CHANGE_NAME: 'changeName',
    CHANGE_PRICE: 'changePrice',
    BACK_TO_MAIN_MENU: 'backToMainMenu',
    SEND_MESSAGE: 'sendMessage',
    EXCLAMATION: 'exclamation',
    YES: 'yes',
    NO: 'no',
    LINK: 'LINK',
    NOTIFICATION: 'NOTIFICATION',
    RESERVED: 'RESERVED',
    LIST: 'LIST',
    DETAIL: 'DETAIL',
    DOWN: 'DOWN',
    USD: 'USD',
    EUR: 'EUR',
    CARD: 'CARD',
    MONEY: 'MONEY',
    FIND: 'FIND',
    OPEN: 'OPEN',
    MANAGER: 'MANAGER',
    NEW_ADMIN: 'NEW_ADMIN'
}

const CURRENCY_EVENT = {
    SET_CURRENCY_BUY: 'SET_CURRENCY_BUY',
    SET_CURRENCY_SELL: 'SET_CURRENCY_SELL',
    SET_CURRENCY_MIN_SUM: 'SET_CURRENCY_MIN_SUM',
    SET_CURRENCY_RESERVE: 'SET_CURRENCY_RESERVE',
    SET_CURRENCY_MULTIPLIER_BUY: 'SET_CURRENCY_MULTIPLIER_BUY',
    SET_CURRENCY_MULTIPLIER_SELL: 'SET_CURRENCY_MULTIPLIER_SELL'
}

const ORDERS_EVENTS = {
    ORDER_INFO: 'ORDER_INFO',
    ORDER_REJECT: 'ORDER_REJECT',
    ORDER_CONFIRM: 'ORDER_CONFIRM',
    SET_CURRENCY_FOR_EXCHANGE: 'SET_CURRENCY_FOR_EXCHANGE',
    ORDER_INPUT_AMOUNT_AWAIT: 'ORDER_INPUT_AMOUNT_AWAIT',
    ORDER_INPUT_WALLET_AWAIT: 'ORDER_INPUT_WALLET_AWAIT',
    SELECT_NETWORK: 'SELECT_NETWORK',
    PAYMENT_COMPLEATE: 'PAYMENT_COMPLEATE',
    CANCEL: 'CANCEL'
}

const MAIN_MENU_UI_CONTROLS_EVENT = {
    CREATE_EXCHANGE_REQUEST: 'CREATE_EXCHANGE_REQUEST',
    SET_CURRENCY_VALUES_CONFIG: 'SET_CURRENCY_VALUES_CONFIG',
    CHAT: 'CHAT',
    BOT_INFO: 'BOT_INFO',
    NOTIFICATION: 'NOTIFICATION',
    GET_CURRENCY_VALUES: 'GET_CURRENCY_VALUES',
    GET_CURRENCY_LIST: 'GET_CURRENCY_LIST',
    GET_PENDING_ORDERS_DATA: 'GET_PENDING_ORDERS_DATA',
    CREATE_NEW_ORDER: 'CREATE_NEW_ORDER',
    FIND_ORDER: 'FIND_ORDER',
    GET_ORDERS_DATA: 'GET_ORDERS_DATA',
    ADD_NEW_ADMIN: 'ADD_NEW_ADMIN',
    CHANGE_CONTACT_MANAGER: 'CHANGE_CONTACT_MANAGER',
    WALLETS: 'WALLETS',
    CHANGE_WALLET: 'CHANGE_WALLET'
}

const USER_ROLES = {
    ADMIN: 'admin',
    WORKER: 'worker',
    VBIVER: 'vbiver',
    REJECTED: 'rejected'
}

const CURRENCY_NAMES = {
    USDT: 'USDT',
    USDC: 'USDC',
    EUR: 'EUR',
    USD: 'USD',
    BTC: 'BTC',
    ETH: 'ETH',
    UAH: 'UAH',
}

const NETWORK_TYPES = {
    ERC_20: 'ERC-20',
    ERC_2: 'ERC-2',
    BEP_20: 'BEP-20'
}

const EMPTY_EVENT = 'VOID';

// PROD token
const TOKEN = "5835524201:AAELin9i9ZXASMtObwVA9RGiUD8IJ2joBFA";
// TEST token
// const TOKEN = "6087484133:AAEDMGJZ0wXk2lSSPTRNwEKtGpheJMZRLfs";

export {
    EMOJI_NAMES,
    TOKEN,
    SERVER_URL,
    MAIN_MENU_UI_CONTROLS_EVENT,
    CURRENCY_EVENT,
    CURRENCY_NAMES,
    ORDERS_EVENTS,
    USER_ROLES,
    EMPTY_EVENT,
    NETWORK_TYPES
}